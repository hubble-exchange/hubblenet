...
var (
	tests = map[string]testutils.PrecompileTest{
		"insufficient gas for getCrossMarginAccountData should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := GetCrossMarginAccountDataInput{}
				input, err := PackGetCrossMarginAccountData(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: GetCrossMarginAccountDataGasCost - 1,
			ReadOnly:    false,
		...
		"insufficient gas for getTotalFundingForCrossMarginPositions should fail": {
			...
			InputFn: func(t testing.TB) []byte {
				...
				input, err := PackGetTotalFundingForCrossMarginPositions(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: GetTotalFundingForCrossMarginPositionsGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
		"insufficient gas for getTraderDataForMarket should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
...
// TestTraderViewerRun tests the Run function of the precompile contract.
func TestTraderViewerRun(t *testing.T) {
	// Run tests.
...
func BenchmarkTraderViewer(b *testing.B) {
	// Benchmark tests.
	for name, test := range tests {
