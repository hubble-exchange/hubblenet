...
func GetSignedOrderFilledAmount(stateDB contract.StateDB, orderHash [32]byte) *big.Int {
	...
	num := stateDB.GetState(signedOrderBookAddress(stateDB), common.BigToHash(orderInfo)).Bytes()
	return fromTwosComplement(num)
}

func GetSignedOrderStatus(stateDB contract.StateDB, orderHash [32]byte) int64 {
	orderInfo := signedOrderInfoMappingStorageSlot(orderHash)
	return new(big.Int).SetBytes(stateDB.GetState(signedOrderBookAddress(stateDB), common.BigToHash(new(big.Int).Add(orderInfo, big.NewInt(1)))).Bytes()).Int64()
}
...
func signedOrderBookAddress(stateDB contract.StateDB) common.Address {
