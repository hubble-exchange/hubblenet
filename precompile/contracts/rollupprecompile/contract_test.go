...
func TestRun(t *testing.T) {
	tests := map[string]testutils.PrecompileTest{
		"insufficient gas for getOrderHash should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// set test input to a value here
				var testInput IOrderBookRollupOrder
				input, err := PackGetOrderHash(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: GetOrderHashGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
		"insufficient gas for isTradingAuthority should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := IsTradingAuthorityInput{}
				input, err := PackIsTradingAuthority(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: IsTradingAuthorityGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
		"insufficient gas for validateCancelOrders should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := ValidateCancelOrdersInput{}
				input, err := PackValidateCancelOrders(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: ValidateCancelOrdersGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
		...
		"insufficient gas for validateOrder should fail": {
			Caller: common.Address{1},
			InputFn: func(t testing.TB) []byte {
				// CUSTOM CODE STARTS HERE
				// populate test input here
				testInput := ValidateOrderInput{}
				input, err := PackValidateOrder(testInput)
				require.NoError(t, err)
				return input
			},
			SuppliedGas: ValidateOrderGasCost - 1,
			ReadOnly:    false,
			ExpectedErr: vmerrs.ErrOutOfGas.Error(),
		},
