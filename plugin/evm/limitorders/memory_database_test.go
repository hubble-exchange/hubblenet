...
func TestAdd(t *testing.T) {
	...
	limitOrder, orderId := createLimitOrder(positionType, userAddress, baseAssetQuantity, price, status, signature, blockNumber, salt, time.Minute)
...
func TestGetFulfillableOrders(t *testing.T) {
	t.Run("Long orders", func(t *testing.T) {
		...
		for i := uint64(0); i < 3; i++ {
			...
			limitOrder, orderId := createLimitOrder("short", userAddress, baseAssetQuantity, price, status, signature, blockNumber, salt, time.Minute)
			inMemoryDatabase.Add(orderId, &limitOrder)
		}
		...
		longOrder1, orderId := createLimitOrder("long", userAddress, longOrderBaseAssetQuantity, price1, status, signature1, blockNumber1, salt1, time.Minute)
		...
		longOrder2, orderId := createLimitOrder("long", userAddress, longOrderBaseAssetQuantity, price2, status, signature2, blockNumber2, salt2, time.Minute)
		...
		returnedLongOrders, _ := inMemoryDatabase.GetFulfillableOrders(AvaxPerp, uint64(time.Now().Unix()))
	...
	t.Run("Short orders", func(t *testing.T) {
		...
		for i := uint64(0); i < totalLongOrders; i++ {
			signature := []byte("signature")
			salt := big.NewInt(0).Add(big.NewInt(int64(i)), big.NewInt(time.Now().Unix()))
			limitOrder, orderId := createLimitOrder("long", userAddress, longOrderBaseAssetQuantity, longOrderPrice, status, signature, blockNumber, salt, time.Minute)
			inMemoryDatabase.Add(orderId, &limitOrder)
		}
		...
		blockNumber1 := big.NewInt(2)
		salt1 := big.NewInt(time.Now().Unix())
		shortOrder1, orderId := createLimitOrder("short", userAddress, baseAssetQuantity, price1, status, signature1, blockNumber1, salt1, time.Minute)
		...
		salt2 := big.NewInt(0).Add(salt1, big.NewInt(1))
		shortOrder2, orderId := createLimitOrder("short", userAddress, baseAssetQuantity, price2, status, signature2, blockNumber2, salt2, time.Minute)
		...
		_, returnedShortOrders := inMemoryDatabase.GetFulfillableOrders(AvaxPerp, uint64(time.Now().Unix()))
	...
	t.Run("Expired orders", func(t *testing.T) {
		// create a short order with expiry of 1 second, a long order with expiry of 5 seconds. Then after 1 second, GetFulfillableOrders returns only the long order
		inMemoryDatabase := NewInMemoryDatabase()
		id1 := uint64(1)
		baseAssetQuantity := big.NewInt(-20)
		signature1 := []byte(fmt.Sprintf("Signature short order is %d", id1))
		price1 := big.NewInt(10)
		blockNumber1 := big.NewInt(2)
		salt1 := big.NewInt(time.Now().Unix())
		shortOrder1, orderId := createLimitOrder("short", userAddress, baseAssetQuantity, price1, status, signature1, blockNumber1, salt1, time.Second)
		inMemoryDatabase.Add(orderId, &shortOrder1)
...
func createLimitOrder(positionType string, userAddress string, baseAssetQuantity *big.Int, price *big.Int, status Status, signature []byte, blockNumber *big.Int, salt *big.Int, expiry time.Duration) (LimitOrder, common.Hash) {
	lo := LimitOrder{
		...
		Expiry:                  big.NewInt(time.Now().Add(expiry).Unix()),
	}
...
func TestGetUnfilledBaseAssetQuantity(t *testing.T) {
	t.Run("When limit FilledBaseAssetQuantity is zero, it returns BaseAssetQuantity", func(t *testing.T) {
		...
		salt1 := big.NewInt(time.Now().Unix())
		longOrder, _ := createLimitOrder("long", "0x22Bb736b64A0b4D4081E103f83bccF864F0404aa", baseAssetQuantityLongOrder, big.NewInt(21), Placed, signature, big.NewInt(2), salt1, time.Minute)
		...
		salt2 := big.NewInt(0).Add(salt1, big.NewInt(1))
		shortOrder, _ := createLimitOrder("short", "0x22Bb736b64A0b4D4081E103f83bccF864F0404aa", baseAssetQuantityShortOrder, big.NewInt(21), Placed, signature, big.NewInt(2), salt2, time.Minute)
...
func addLimitOrder(db *InMemoryDatabase) common.Hash {
	...
	limitOrder, orderId := createLimitOrder(positionType, userAddress, big.NewInt(50), price, status, signature, blockNumber, salt, time.Minute)
